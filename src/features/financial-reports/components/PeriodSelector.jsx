// C:\python\django\somane_frontend\src\features\financial-reports\components\PeriodSelector.jsx
...
function computeDisplayStatus(lockDate, status) {
  if (!lockDate) return status || 'OPEN';
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  return new Date(lockDate) <= today ? 'LOCKED' : (status || 'OPEN');
}

const EMPTY_FORM = {
  code: '', name: '', period_type: 'ANNUAL', fiscal_year: '',
  date_start: '', date_end: '', finance_account_framework: '',
  previous_period: '', opening_reference_period: '',
  lock_date: '', comment: '', active: true,
};
...
export default function PeriodsList() {
  ...
  useEffect(() => { loadData(); loadFrameworks(); }, [activeEntity]);
  ...
  const loadFrameworks = async () => {
    setFwLoading(true);
    try {
      const res = await apiClient.get('compta/frameworks/');
      const data = Array.isArray(res) ? res : res.results || [];
      setFrameworks(data.filter(fw => fw.active !== false));
    } catch { console.warn('Impossible de charger les référentiels comptables'); }
  ...
  const computedStatus = useMemo(
    () => computeDisplayStatus(formData.lock_date, editingPeriod?.status),
    [formData.lock_date, editingPeriod?.status]
  );

  const openCreate = () => {
    if (!activeEntity) { setError('Veuillez sélectionner une entité'); return; }
    setEditingPeriod(null); setFormData(EMPTY_FORM);
    setShowAdvanced(false); setError(''); setShowModal(true);
  };
  ...
  const handleEdit = (period) => {
    ...
    setFormData({
      ...
      finance_account_framework: period.finance_account_framework?.id
      ...
      lock_date: period.lock_date || '', comment: period.comment || '',
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.date_start >= formData.date_end) { setError('La date de fin doit être après la date de début'); setSaving(false); return; }
    ...
    const payload = {
      ...
      finance_account_framework: formData.finance_account_framework || null,
    ...
    delete payload.status; // Le backend calcule le statut dans save()
    ...
    try {
      ...
      if (be) {
        if (typeof be === 'object') {
          const k = Object.keys(be)[0];
          const v = be[k];
          msg = Array.isArray(v) ? v[0] : String(v);
        } else { msg = be.detail || String(be); }
      }
      setError(msg);
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 mt-1 text-sm">Gérez les exercices et leurs liaisons N / N-1</p>
        </div>
        <button onClick={openCreate} disabled={!activeEntity}
          ...
          <FiPlus size={16} /> Nouvelle période
        </button>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-x-auto shadow-sm">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              {['Code','Nom','Type','Exercice','Début → Fin','Référentiel','N-1 liée','Statut','Actions'].map(h => (
          ...
          <tbody className="divide-y divide-gray-100">
            {periods.length === 0 ? (
              <tr><td colSpan={9} className="px-4 py-12 text-center text-gray-400">Aucune période trouvée</td></tr>
            ) : periods.map((period) => {
              const displayStatus = computeDisplayStatus(period.lock_date, period.status);
              const statusCfg     = STATUS_DISPLAY[displayStatus] || STATUS_DISPLAY.OPEN;
              const isLocked      = displayStatus === 'LOCKED';
              const n1Id = period.opening_reference_period || period.previous_period;
              const n1   = n1Id ? periods.find(p => String(p.id) === String(n1Id)) : null;
              const fwId = period.finance_account_framework?.id || period.finance_account_framework;
              ...
              return (
                <tr key={period.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    {fw ? (
                      <span className="px-2 py-0.5 bg-slate-100 text-slate-700 rounded text-xs font-mono" title={fw.name}>
                        {fw.code}
                      </span>
                    ) : fwId ? (
                      <span className="text-xs text-gray-400">#{fwId}</span>
                  ...
                  <td className="px-4 py-3">
                    {n1 ? (
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="font-mono text-xs text-indigo-700">{n1.code}</span>
                        {hasValidatedImport(n1.id) ? (
                  ...
                  <td className="px-4 py-3">
                    <div className="flex justify-center gap-1">
                      ...
                      <button onClick={() => handleDelete(period.id)} disabled={isLocked}
          ...
          <div className="bg-white rounded-xl w-full max-w-2xl max-h-[92vh] overflow-y-auto shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="px-6 py-5 space-y-5">
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 text-sm text-slate-700 flex items-center gap-2">
                ...
                <span><strong>Entité :</strong> {activeEntity?.raison_sociale || activeEntity?.code || '—'}</span>
              </div>
              ...
              <div className="flex items-center gap-3 p-3 bg-gray-50 border border-gray-200 rounded-lg">
                ...
                <div className="flex-1 text-sm text-gray-600">
                  ...
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold ml-2"
                    style={{ background: STATUS_DISPLAY[computedStatus]?.bg, color: STATUS_DISPLAY[computedStatus]?.color }}>
                    {computedStatus === 'LOCKED' && <FiLock size={9} />}
                    {STATUS_DISPLAY[computedStatus]?.label}
                  </span>
                </div>
                <span className="text-xs text-gray-400">
                  {computedStatus === 'LOCKED' ? 'Date de verrouillage atteinte'
                    : editingPeriod ? 'Statut conservé' : 'Ouvert à la création'}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={formData.code} onChange={e => field('code', e.target.value)}
                    required placeholder="Ex : FY2025"
                ...
                <div>
                  ...
                  <input type="text" value={formData.name} onChange={e => field('name', e.target.value)}
                    required placeholder="Ex : Exercice 2025"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={formData.period_type} onChange={e => field('period_type', e.target.value)}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">Exercice comptable <span className="text-red-500">*</span></label>
                  <input type="text" value={formData.fiscal_year} onChange={e => field('fiscal_year', e.target.value)}
                    required placeholder="Ex : 2025"
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-violet-500 text-sm outline-none" />
                </div>
              </div>

              {/* Référentiel comptable — API */}
              <div>
                ...
                {fwLoading ? (
                  ...
                  <select value={formData.finance_account_framework}
                    ...
                    {frameworks.length === 0
                      ...
                      : frameworks.map(fw => (
                          <option key={fw.id} value={fw.id}>
                            {fw.code} — {fw.name}{fw.version ? ` (${fw.version})` : ''}
                          </option>
                        ))
                    }
                  </select>
                )}
                {frameworks.length === 0 && !fwLoading && (
                  <p className="text-xs text-amber-600 mt-1">
                    Aucun référentiel chargé — vérifiez l'endpoint
                    <code className="mx-1 font-mono text-xs">comptabilite/frameworks/</code>
                  </p>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="date" value={formData.date_start} onChange={e => field('date_start', e.target.value)}
                    required className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-violet-500 text-sm outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">Date de fin <span className="text-red-500">*</span></label>
                  <input type="date" value={formData.date_end} onChange={e => field('date_end', e.target.value)}
                    required className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-violet-500 text-sm outline-none" />
                </div>
              </div>
              ...
              <div>
                ...
                <select value={formData.previous_period} onChange={e => field('previous_period', e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-violet-500 text-sm outline-none">
                  <option value="">— Aucune (première balance) —</option>
                  {periods.filter(p => !editingPeriod || p.id !== editingPeriod.id).map(p => (
                    <option key={p.id} value={p.id}>
                      {p.code} — {p.name}{hasValidatedImport(p.id) ? ' ✓' : ' ⚠ non importée'}
                    </option>
                  ))}
                </select>
                <p className="text-xs text-gray-400 mt-1">
                  Détermine automatiquement la balance N-1 lors de l'import. Si vide → première balance, Règle 2 désactivée.
                </p>
                {formData.previous_period && !hasValidatedImport(formData.previous_period) && (
                  <div className="mt-2 flex items-start gap-2 p-2.5 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-700">
                    ...
                    <span>Cette période N-1 n'a pas encore de balance validée. L'import sera bloqué jusqu'à ce qu'elle soit importée et validée.</span>
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                <input type="date" value={formData.lock_date} onChange={e => field('lock_date', e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-violet-500 text-sm outline-none" />
                <p className="text-xs text-gray-400 mt-1">
              ...
              <div className="border border-gray-200 rounded-lg overflow-hidden">
                ...
                {showAdvanced && (
                  <div className="px-4 py-4 space-y-4 bg-white">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1.5">Référence d'ouverture spécifique</label>
                      <select value={formData.opening_reference_period}
                      ...
                      <p className="text-xs text-gray-400 mt-1">Prioritaire sur la période N-1. À utiliser uniquement en cas de reprise de données.</p>
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      <label htmlFor="active" className="text-sm text-gray-700">Période active</label>
                    </div>
                    <div>
                      ...
                      <textarea value={formData.comment} onChange={e => field('comment', e.target.value)}
                        rows={3} placeholder="Remarques ou informations complémentaires…"
              ...
              <div className="flex justify-end gap-3 pt-2 border-t border-gray-100">
                <button type="button" onClick={() => setShowModal(false)} disabled={saving}
                  className="px-5 py-2 text-sm text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 disabled:opacity-50">
                  Annuler
                </button>
                <button type="submit" disabled={saving || !activeEntity}
                  className="px-6 py-2 text-sm bg-violet-600 text-white rounded-lg hover:bg-violet-700 disabled:opacity-50 flex items-center gap-2">

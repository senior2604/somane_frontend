...
export default function CommandesClientEdit() {
  const { id } = useParams();
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [commandeRes, clientsRes, devisesRes, societesRes, utilisateursRes] = await Promise.all([
      ...
      setLignes(commandeRes.data.lignes || []);
  ...
  const calculerMontantHT = useCallback((quantite, prixUnitaire, remise) => {
    const qty = parseFloat(quantite) || 0;
    const prix = parseFloat(prixUnitaire) || 0;
    const remiseVal = parseFloat(remise) || 0;
    const montant = qty * prix;
    const montantRemise = montant * (remiseVal / 100);
    return (montant - montantRemise).toFixed(2);
  }, []);
  ...
  const { totalHT, totalTTC } = useMemo(() => {
    ...
    lignes.forEach(ligne => {
      const montantHT = parseFloat(ligne.montant_ht) || 0;
    ...
    if (formData.amount_total !== ttc) {
      setFormData(prev => ({ ...prev, amount_total: ttc }));
    }
    
    return { totalHT: ht, totalTTC: ttc };
  }, [lignes, formData.amount_total]);
  ...
  const handleLigneChange = (index, field, value) => {
    ...
    if (field === 'quantite' || field === 'prix_unitaire' || field === 'remise') {
      ...
      const montantHT = calculerMontantHT(
        newLignes[index].quantite,
        newLignes[index].prix_unitaire,
        newLignes[index].remise
      );
      newLignes[index].montant_ht = montantHT;
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await apiClient.put(`/ventes/commandes-client/${id}/`, { ...formData, lignes });
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      ...
      <div className="flex items-center gap-3 mb-4">
        <button
          onClick={() => navigate('/ventes/commandes-client')}
          className="text-blue-600 hover:text-blue-800 text-sm font-medium"
        >
          Modifier
        </button>
        <span className="text-gray-400">|</span>
        <h1 className="text-lg font-semibold text-gray-900">Commandes Clients</h1>
      </div>
      ...
      <div className="bg-white border border-gray-300 rounded-lg overflow-hidden mb-6">
        ...
        <div className="border-b border-gray-300 p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            ...
            <div>
              <p className="text-xs font-medium text-gray-700 mb-1">Date commande *</p>
              <input
                type="date"
                name="date_order"
            ...
            <div>
              <p className="text-xs font-medium text-gray-700 mb-1">Devise *</p>
              <select
                name="currency_id"
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
            ...
            <div>
              <p className="text-xs font-medium text-gray-700 mb-1">Facturation</p>
              <select
                name="invoice_status"
              ...
              >
                ...
                <option value="partiellement_facturee">Partielle</option>
              </select>
            </div>
            
            <div>
              <p className="text-xs font-medium text-gray-700 mb-1">Livraison</p>
              <select
              ...
              >
                ...
                <option value="partiellement_livree">Partielle</option>
              </select>
            </div>
            
            <div>
              <p className="text-xs font-medium text-gray-700 mb-1">Montant TTC</p>
              <input
                type="text"
                value={formatCurrency(totalTTC)}
                readOnly
                className="w-full px-2 py-1.5 text-xs border border-gray-300 rounded bg-gray-50 text-gray-900 font-medium"
              />
            </div>
          </div>
        </div>
                ...
                <table className="w-full text-xs border-collapse">
                  <thead className="bg-gray-50">
                    <tr className="text-left">
                      <th className="px-2 py-2 font-medium text-gray-700 border border-gray-300">Produit</th>
                      ...
                      <th className="px-2 py-2 font-medium text-gray-700 border border-gray-300 text-right">Prix U.</th>
                      <th className="px-2 py-2 font-medium text-gray-700 border border-gray-300 text-right">Remise %</th>
                      <th className="px-2 py-2 font-medium text-gray-700 border border-gray-300">TVA %</th>
                      <th className="px-2 py-2 font-medium text-gray-700 border border-gray-300 text-right">Montant HT</th>
                      <th className="px-2 py-2 font-medium text-gray-700 border border-gray-300">Date Liv.</th>
                  ...
                  <tbody>
                    {lignes.map((ligne, index) => (
                      <tr key={ligne.id}>
                        ...
                        <td className="px-2 py-1.5 border border-gray-300">
                          <div className="w-full px-1 py-0.5 text-xs text-right font-medium">{formatCurrency(parseFloat(ligne.montant_ht) || 0)}</div>
                        </td>
                        <td className="px-2 py-1.5 border border-gray-300">
                          <input
                            type="date"
              ...
              <div className="flex justify-between items-center pt-4 border-t border-gray-300">
                ...
                <div className="space-y-1">
                  <div className="text-xs text-gray-600">
                    <span>Total HT: </span>
                    <span className="font-bold">{formatCurrency(totalHT)} XOF</span>
                  </div>
                  <div className="text-sm font-bold text-gray-900 bg-green-50 px-3 py-1 rounded">
                    Total TTC: {formatCurrency(totalTTC)} XOF
                  </div>
                </div>
              </div>
            </div>

...
const AmountInput = ({ value, onChange, disabled = false }) => {
  ...
  return (
    <input
      ...
      disabled={disabled}
      className="h-[26px] w-full border border-gray-300 px-2 text-left text-xs hover:border-purple-400 focus:border-purple-600 disabled:bg-gray-100"
      placeholder="0"
    />
  );
};
        ...
        <div className={`grid gap-0 ${showTraceabilityPanel ? 'md:grid-cols-[minmax(0,1fr)_320px]' : 'grid-cols-1'}`}>
          <div className="min-w-0">
            ...
            <div className="flex items-center justify-between border-b border-gray-300 px-4 py-2">
              ...
              <div className="flex flex-wrap items-center justify-end gap-2">
                <PaymentProcess state={saving ? 'processing' : isPosted ? 'paid' : 'draft'} />
              </div>
            </div>
            ...
            <div className="border-b border-gray-300 px-4 py-3">
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  {formData.payment_type === 'transfer' && <Field label="Journal destination" required><SearchSelect value={formData.destination_journal_id} onChange={(value) => setField('destination_journal_id', value)} disabled={!isDraft} options={treasuryJournals.length ? treasuryJournals : options.journals} getLabel={(journal) => optionLabel(journal)} placeholder="Journal destination" /></Field>}
            ...
            <div className="p-4">
              ...
              <textarea value={formData.narration || ''} onChange={(event) => setField('narration', event.target.value)} disabled={!isDraft} rows={6} className="w-full border border-gray-300 px-3 py-2 text-xs outline-none focus:border-purple-600 disabled:bg-gray-100" placeholder="Notes / libelle du paiement..." />
            </div>
          </div>

// src/features/comptabilité/pages/TaxGroups/Create.jsx
...
import { 
  ...
  FiDollarSign, FiLayers, FiFileText, FiCopy, FiRotateCcw, FiGlobe, FiHash
} from 'react-icons/fi';
...
// ==========================================
// COMPOSANT AUTOCOMPLETE AVEC LOGS
// ==========================================
...
// COMPOSANT PRINCIPAL
// ==========================================
export default function TaxGroupsCreate() {
  ...
  const loadOptions = useCallback(async () => {
    ...
    try {
      console.log('🔄 Chargement des comptes pour entité:', activeEntity.id);
      ...
      console.log(`📊 ${operationalAccounts.length} comptes opérationnels chargés`);
  ...
  const loadGroupData = useCallback(async (id) => {
    setIsLoading(true);
    try {
      console.log('🔄 Chargement du groupe ID:', id);
      const groupRes = await apiClient.get(`/compta/tax-groups/${id}/`);
      const group = groupRes;
      
      console.log('📦 Données du groupe reçues:', {
        name: group.name,
  ...
  useEffect(() => {
    ...
    if (idFromUrl && !groupId) {
      loadGroupData(idFromUrl);
    }
  }, [location.search, loadGroupData, groupId]);
  ...
  // ============================================================
  // 🔧 CORRECTION CRITIQUE ICI : AJOUT DE _id DANS LES CHAMPS
  // ============================================================
  const handleSave = async (silent = false) => {
    ...
    try {
      // ✅ CORRECTION : Les champs doivent se terminer par _id
      const payload = {
        name: formData.name,
      ...
      console.log('🔍 Détails des comptes (avec _id):');
      console.log('  - property_tax_payable_account_id:', payload.property_tax_payable_account_id);
      console.log('  - property_tax_receivable_account_id:', payload.property_tax_receivable_account_id);
      console.log('  - property_advance_tax_payment_account_id:', payload.property_advance_tax_payment_account_id);
      
      let result;
      if (groupId) {
        console.log('📝 Mise à jour du groupe ID:', groupId);
        result = await apiClient.put(`/compta/tax-groups/${groupId}/`, payload);
      } else {
        console.log('📝 Création d\'un nouveau groupe');
        result = await apiClient.post('/compta/tax-groups/', payload);
        if (result?.id) setGroupId(result.id);
      }
      
      console.log('✅ Réponse API:', result);
      
      if (!silent) setSuccess(groupId ? 'Groupe modifié avec succès !' : 'Groupe créé avec succès !');
      setHasUnsavedChanges(false);
      ...
      <div className="min-h-screen bg-gray-50 p-4">
        <div className="max-w-4xl mx-auto">
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-center">
            ...
            <p className="text-sm text-gray-600 mb-4">Veuillez sélectionner une entité pour créer un groupe de taxes.</p>
              ...
              <div className="flex flex-col h-12 justify-center">
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <span className="text-xs text-gray-600 font-medium">
                    {groupId ? `Groupe: ${formData.name || 'Modification'}` : (formData.name ? `Groupe: ${formData.name}` : 'Nouveau groupe de taxes')}
                  </span>
                  {activeEntity && (
              ...
              <div className="flex items-center" style={{ height: '26px' }}>
                ...
                <div className="flex-1 ml-2 border border-gray-300 hover:border-purple-400 transition-colors" style={{ height: '26px' }}>
                  <AutocompleteInput 
                    ...
                    onSelect={(id, label) => { 
                      console.log('🎯 Selection compte payable:', { id, label });
              ...
              <div className="flex items-center" style={{ height: '26px' }}>
                ...
                <div className="flex-1 ml-2 border border-gray-300 hover:border-purple-400 transition-colors" style={{ height: '26px' }}>
                  <AutocompleteInput 
                    ...
                    onSelect={(id, label) => { 
                      console.log('🎯 Selection compte recevable:', { id, label });
              ...
              <div className="flex items-center" style={{ height: '26px' }}>
                ...
                <div className="flex-1 ml-2 border border-gray-300 hover:border-purple-400 transition-colors" style={{ height: '26px' }}>
                  <AutocompleteInput 
                    ...
                    onSelect={(id, label) => { 
                      console.log('🎯 Selection compte acomptes:', { id, label });

...
export default function GrandLivre() {
  ...
  const columns = useMemo(() => [
    { id: 'account', label: 'Compte / Ecriture', width: 'min-w-[260px]' },
    { id: 'date', label: 'Date', width: 'w-[95px]' },
    { id: 'journal', label: 'Journal', width: 'w-[85px]' },
    { id: 'move', label: 'Piece', width: 'w-[130px]' },
    { id: 'reference', label: 'Reference', width: 'w-[130px]' },
    { id: 'partner', label: 'Partenaire', width: 'min-w-[160px]' },
    { id: 'label', label: 'Libelle', width: 'min-w-[180px]' },
  ...
  const loadGrandLivre = useCallback(async () => {
    ...
    try {
      ...
      setReport({ ...(data || {}), results: nextResults });

      const nextExpanded = {};
      nextResults.forEach((account) => {
  ...
  const activeFilterChips = useMemo(() => {
    ...
    if (filters.state !== 'all') chips.push({ key: 'state', label: `Etat: ${filters.state === 'posted' ? 'Comptabilisees' : filters.state}`, cls: 'bg-green-100 text-green-700' });
                      ...
                      {expanded && lines.map((line) => {
                        ...
                        return (
                          <tr key={line.id} className="hover:bg-gray-50 transition-colors">
                            {visibleColumns.account && (
                              <td className="border border-gray-300 px-2 py-1.5 text-xs text-gray-700 pl-8">
                                <div className="flex items-center gap-1">
                                  <div className="truncate max-w-[260px]" title={`${line.move_name || ''} ${lineLabel}`.trim()}>
                            ...
                            {visibleColumns.date && (
                              <td className="border border-gray-300 px-2 py-1.5 text-xs text-gray-700">{formatDate(line.date)}</td>
                            )}
                            ...
                            {visibleColumns.move && (
                              <td className="border border-gray-300 px-2 py-1.5 text-xs text-gray-700">
                                <div className="truncate max-w-[130px]" title={line.move_name || ''}>
                                  {line.move_name || '-'}
                                </div>
                              </td>
                            )}
                            {visibleColumns.reference && (
                              <td className="border border-gray-300 px-2 py-1.5 text-xs text-gray-700">
                                <div className="truncate max-w-[130px]" title={line.ref || line.move_ref || line.reference || ''}>
                            ...
                            {visibleColumns.partner && (
                              <td className="border border-gray-300 px-2 py-1.5 text-xs text-gray-700">
                                <div className="truncate max-w-[180px]" title={line.partner_name || ''}>{line.partner_name || '-'}</div>
                              </td>
                            )}
                            {visibleColumns.label && (
                              <td className="border border-gray-300 px-2 py-1.5 text-xs text-gray-700">
                                <div className="truncate max-w-[220px]" title={lineLabel}>
                                  {lineLabel || '-'}
                                </div>
                              </td>
                            )}

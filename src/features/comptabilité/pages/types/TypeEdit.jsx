// src/features/comptabilite/pages/types/TypeEdit.jsx
...
function TypeEdit() {
  ...
  const { fetchTypeById, updateType } = useTypeStore();
  ...
  const initPage = useCallback(async () => {
    ...
    try {
      ...
      const fwId = relationId(type.framework);
      ...
      form.setFieldsValue({
        ...
        code: type.code || '',
        name: type.name || '',
        internal_group: relationId(type.internal_group),
        parent: relationId(type.parent),
        default_balance_type: type.default_balance_type || 'debit',
        default_debit: type.default_debit ?? type.default_balance_type !== 'credit',
        default_credit: type.default_credit ?? type.default_balance_type === 'credit',
        allow_reconciliation: type.allow_reconciliation ?? false,
        include_in_opening_balance: type.include_in_opening_balance ?? true,
        closing_behavior: type.closing_behavior || 'none',
        active: type.active !== false,
        note: type.note || '',
      });
    } catch (caughtError) {
      console.error(caughtError);
      message.error('Impossible de charger la nature à modifier');
      navigate('/comptabilite/types');
    } finally {
      setLoading(false);
    }
  }, [fetchFrameworks, fetchTypeById, form, id, loadFrameworkData, navigate]);
  ...
  const parentTypeOptions = useMemo(() => (
    parentTypes
      .filter((type) => String(type.id) !== String(id))
      .map((type) => ({
        value: type.id,
  ...
  const saveValues = async (values, redirect = { to: '/comptabilite/types', options: undefined }) => {
    setLoading(true);
    setError(null);
    setSuccess(null);
    try {
      await updateType(id, values);
      setSuccess('Nature modifiée avec succès');
  ...
  const onFinish = async (values) => {
    await saveValues(values, { to: '/comptabilite/types', options: undefined });
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 p-4">
        <div className="mx-auto flex min-h-[300px] max-w-7xl items-center justify-center border border-gray-300 bg-white">
          <Spin size="large" tip="Chargement de la nature..." />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4">
      <style>{`
        .type-edit-form .ant-form-item {
          margin-bottom: 0;
        }
        .type-edit-form .ant-input {
        ...
        .type-edit-form textarea.ant-input {
          min-height: 110px !important;
          line-height: 1.5 !important;
          resize: vertical;
        }
        .type-edit-form .ant-form-item-explain-error,
        .type-edit-form .ant-form-item-extra {
          font-size: 11px;
          margin-top: 2px;
        }
        .type-edit-form .ant-switch {
      ...
      <div className="mx-auto max-w-7xl border border-gray-300 bg-white">
        <Spin spinning={loading && Boolean(typeData)}>
          <Form
            form={form}
            layout="vertical"
            onFinish={onFinish}
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex min-w-0 items-start gap-3">
                  ...
                  <div className="flex min-h-[48px] min-w-0 flex-col justify-center">
                    ...
                    >
                      Modifier la nature de compte
                    </div>
                    <div className="mt-1 text-xs font-medium text-gray-600">
                      {watchedCode || watchedName
                        ? `${watchedCode || 'Nature'} - ${watchedName || 'Compte'}`
                      ...
                      <div className="absolute right-0 z-50 mt-1 w-52 rounded-sm border border-gray-300 bg-white shadow-lg">
                        <button
                          ...
                          onClick={() => {
                            ...
                            navigateWithGuard(`/comptabilite/types/${id}`);
                          }}

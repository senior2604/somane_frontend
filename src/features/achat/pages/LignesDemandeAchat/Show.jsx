...
export default function LignesDemandeAchatShow() {
  const { id } = useParams();
  ...
  if (!item) {
    return (
      <div className="p-6 text-center">
        ...
        <button onClick={() => navigate('/achats/lignes-demande-achat')} className="text-violet-600 hover:text-violet-800 mt-4">
          Retour à la liste
        </button>
      </div>
    );
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-600 mt-1">Détails de la ligne de demande d'achat</p>
          </div>
        </div>
        <div className="flex gap-2">
          <button
            onClick={() => navigate(`/achats/lignes-demande-achat/${id}/edit`)}
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <p className="text-sm text-gray-600">Demande d'Achat</p>
                <p className="text-lg font-medium text-gray-900">
                  {getReferenceName(demandesAchat, item.demande_achat_id)}
                </p>
              </div>
              <div>

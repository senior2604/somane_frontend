// features/achat/components/AchatTableContainer.jsx
...
export default function AchatTableContainer({
  data = [],
  loading = false,
  error = null,
  title = "Tableau",
  ...
  const config = moduleConfigs[moduleType] || moduleConfigs.demandes;
      ...
      <div className="px-4 pt-0 pb-4 bg-gradient-to-br from-gray-50 to-white min-h-screen">
        <div className="flex flex-col items-center justify-center h-96">
          <div className="relative">
            ...
            <div className="absolute top-0 left-0 w-12 h-12 border-3 border-green-600 border-t-transparent rounded-full animate-spin"></div>
          </div>
          <div className="mt-4">
    ...
    <div className="px-4 pt-0 pb-4 bg-gradient-to-br from-gray-50 to-white min-h-screen">
      ...
      <div className="flex items-center gap-2 mb-0 flex-wrap p-2 border border-gray-300 rounded-lg bg-white">
        {onCreate && (
          <button 
            onClick={onCreate}
            className="px-3 py-1.5 bg-gradient-to-r from-green-600 to-green-500 text-white rounded hover:from-green-700 hover:to-green-600 transition-all duration-300 font-medium flex items-center gap-1 shadow text-sm"
          >
            <FiPlus size={14} />
            <span>Nouveau</span>
          </button>
        ...
        <button
          ...
          className="text-base font-bold text-gray-900 hover:text-green-700 transition-colors px-2 py-1.5 h-full flex items-center"
        >
          {finalTitle}
        </button>
        ...
        <div className="relative flex-1 max-w-2xl">
          ...
          <input
            ...
            className="pl-9 pr-8 py-1.5 w-full border border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-green-500 focus:border-transparent text-sm h-full"
          ...
          <div className="absolute right-1 top-1/2 transform -translate-y-1/2">
            <button
              ...
              className={`flex items-center gap-1 px-2 py-1.5 rounded text-xs font-medium h-full ${
                activeFilters.length > 0 ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'
        ...
        <select
          ...
          className="ml-auto border border-gray-300 rounded px-2 py-1 text-xs bg-white focus:outline-none focus:ring-1 focus:ring-green-500 focus:border-transparent"
        >
            ...
            {activeFilters.length > 0 && (
              <div className="flex items-center gap-1">
                ...
                {activeFilters.map(filter => (
                  <span 
                    key={filter.id}
                    className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-green-50 text-green-700 text-xs rounded border border-green-200"
                  >
                ...
                <tr className="bg-gradient-to-r from-gray-50 to-gray-100">
                  <th scope="col" className="px-2 py-1.5 text-left text-xs font-semibold text-gray-700 tracking-wider border-r border-gray-300 w-8">
                    <input
                      ...
                      className="w-2.5 h-2.5 text-green-600 rounded focus:ring-green-500 border-gray-300"
                    />
                  </th>
                  ...
                  <td colSpan={visibleColumnDefs.length + 1} className="px-3 py-4 text-center">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      {data.length === 0 && onCreate && (
                        <button 
                          onClick={onCreate}
                          className="px-3 py-1 bg-gradient-to-r from-green-600 to-green-500 text-white rounded hover:from-green-700 hover:to-green-600 transition-all duration-300 font-medium flex items-center gap-1 text-sm"
                        >
                          <FiPlus size={12} />
                ...
                paginatedData.map((item, index) => (
                  <tr 
                    ...
                    className={`transition-all duration-150 cursor-pointer ${
                      item.id === activeRowId
                        ? 'bg-gradient-to-r from-green-100 to-green-50 border-l-4 border-l-green-500'
                        : selectedRows.includes(item.id)
                          ? 'bg-gradient-to-r from-green-50 to-green-25 hover:bg-gradient-to-r hover:from-green-100 hover:to-green-50'
                      ...
                      <button
                        ...
                        className={`min-w-[28px] h-7 rounded border text-xs font-medium transition-all duration-200 ${
                          currentPage === pageNumber
                            ? 'bg-gradient-to-r from-green-600 to-green-500 text-white border-green-600 shadow'
